...
The GS1 VC API is responsible for issuing and verifying Verifiable Credentials as well as creating and resolving
...
- The GS1 **Issuer Agent**: This agent, running on port 5000 by default, contains the did:web of GS1 NL, and is
...
The GS1 VC API, is re-using features exposed by Sphereon's SSI-SDK, amongst which as API's and endpoint functions that
...
say `verification.gs1.nl`, the the value would become `did:web:verification.gs1.nl`. The agent will be able to use these
did:web values even if the DID document is not hosted at https://verification.gs1.nl/.well-known/did.json yet (via copy
...
the `@context` array; most importantly https://ref.gs1.org/gs1/vc/license-context/.
...
```json
{
  "credential": {
    ...
    "credentialStatus": {
      "statusListCredential": "https://verification.gs1.nl/vc/status-lists/1"
    },
    "credentialSubject": {
      "id": "did:web:verification.gs1.nl:did:party_gln:8720796007237",
      "organization": {
        "gs1:partyGLN": "8720796007237",
        "gs1:organizationName": "Test Account",
        "gs1:additionalOrganizationID": [
          {
            "gs1:organizationID": "90001745",
            "gs1:organizationID_Type": "CoC"
          }
        ]
      },
...
```json
{
  "verifiableCredential": {
    ...
    "credentialStatus": {
      "statusListCredential": "https://verification.gs1.nl/vc/status-lists/1",
      "id": "https://verification.gs1.nl/vc/status-lists/1",
...
```json
{
  "verifiableCredential": {
    ...
    "validUntil": "2024-06-22T00:00:00Z",
    "credentialStatus": {
      "statusListCredential": "https://verification.gs1.nl/credentials/status-lists/1",
      "id": "https://verification.gs1.nl/vc/status-lists/1#221154",
      "type": "StatusList2021Entry",
      "statusPurpose": "suspension",
      "statusListIndex": "221154"
    },
...
Please note that the `issuer` DID value does not have to be the GS1 issuer. If however a did:web is used as issuer, the
...
```json
{
  "credentialId": "https://verification.gs1.nl/vc/license/company_prefix/8790171",
  "credentialStatus": [
    {
      "type": "StatusList2021",
      "status": "1"
...
means a REST API is exposed to create DIDs. Since these DIDs will be custodial (eg GS1 NL is managing these DIDs on
...
```json
{
  "did": "did:web:verification.gs1.nl:did:party_gln:12345678",
  "options": {
    "storeSecrets": true
  }
}
...
```json
{
  ...
  "didState": {
    ...
    "didDocument": {
      ...
      "id": "did:web:verification.gs1.nl:did:party_gln:12345678",
      "verificationMethod": [
        {
          "controller": "did:web:verification.gs1.nl:did:party_gln:12345678",
          "id": "did:web:verification.gs1.nl:did:party_gln:12345678#02cf2419d5b12348a7c8249abd69671090d7b5016fe819aa85e886165d18c08fb4",
          "publicKeyJwk": {
            "alg": "ES256",
            "use": "sig",
            "kty": "EC",
...
https://verification.gs1.nl/did/party_gln/12345678/did.json
...
http://localhost:5001/did/identifiers/did:web:verification.gs1.nl:did:party_gln:12345678
...
http://localhost:5001/did/identifiers/did:web:verification.gs1.nl:did:party_gln:12345678?mode=local
...
```json
{
  ...
  "didDocument": {
    ...
    "verificationMethod": [
      {
        "controller": "did:web:verification.gs1.nl:did:party_gln:12345678",
        "id": "did:web:verification.gs1.nl:did:party_gln:12345678#02925110021f5d53468136ad4bf2233596bc8ee22f07c4b37548a346d643fcb73d",
        "publicKeyJwk": {
          "alg": "ES256",
          "use": "sig",
          "kty": "EC",
  ...
  "didDocumentMetadata": {
    "equivalentId": "verification.gs1.nl:did:party_gln:12345678"
  }
}
```
...
is running behind a reverse proxy or load-balancer on the URL `https://verification.gs1.nl`. If you want to test during
