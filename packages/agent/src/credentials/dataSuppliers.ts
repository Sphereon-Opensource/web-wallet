...
export async function defaultCredentialDataSupplier(
  args: CredentialDataSupplierArgs,
): Promise<CredentialDataSupplierResult> {
  const {
  ...
  if (
    ...
    "credentialPayload" in credentialDataSupplierInput &&
    credentialDataSupplierInput.credentialPayload
  ) {
    const credentialPayload =
      credentialDataSupplierInput.credentialPayload as CredentialPayload;
    if (
      types.includes("VerifiableCredential") &&
      !credentialPayload.type?.includes("VerifiableCredential")
    ) {
      credentialPayload.type = [...types];
    } else if (!Array.isArray(credentialPayload.type)) {
      throw Error(
        `Could not infer credential types from offer, or supplied credential payload`,
      );
