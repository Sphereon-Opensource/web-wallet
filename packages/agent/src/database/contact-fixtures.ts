...
export async function addContacts() {
    try {
        ...
        let identifier = await agent.didManagerCreate(existingDidConfig(DIDMethods.DID_JWK, 'w-auth', PRIVATE_DID1_KEY_HEX, {type: 'Secp256r1'}));
        persona1.identities = [toContactIdentityDTO(persona1, identifier)];
        await agent.cmAddContact(persona1);
        // did:jwk:eyJhbGciOiJFUzI1NiIsInVzZSI6InNpZyIsImt0eSI6IkVDIiwiY3J2IjoiUC0yNNTYiLCJ4Ijoid0kzRmhzanpQaHNkYjVSVjdQeFBIS0lmSUZDLXF5T2JYdGQxQWsyeHdHYyIsInkiOiJTbXMyUEhQX05pRW1xWEgxSzdCaTdha2h4cEJMdmp0OOVRRN2E3SkJZOUI0In0
        ...
        identifier = await agent.didManagerCreate(existingDidConfig(DIDMethods.DID_JWK, 'h-auth', PRIVATE_DID2_KEY_HEX, {type: 'Secp256r1'}));
        ...
        identifier = await agent.didManagerCreate(existingDidConfig(DIDMethods.DID_JWK, 'k-auth', PRIVATE_DID3_KEY_HEX, {type: 'Secp256r1'}));
        ...
        identifier = await agent.didManagerCreate(existingDidConfig(DIDMethods.DID_WEB, 'rws-auth', PRIVATE_DID4_KEY_HEX, {traceability: true, alias: 'did:web:localhost:rws', type: 'Secp256r1'}));
        ...
        identifier = await agent.didManagerCreate(existingDidConfig(DIDMethods.DID_WEB, 'stonebase-auth', PRIVATE_DID5_KEY_HEX, {traceability: true, alias: 'did:web:localhost:stonebase', type: 'Secp256r1'}));
        ...
        identifier = await agent.didManagerCreate(existingDidConfig(DIDMethods.DID_WEB, 'sgs-auth', PRIVATE_DID6_KEY_HEX, {traceability: true, alias: 'did:web:localhost:sgs', type: 'Secp256r1'}));
...
function existingDidConfig(method: DIDMethods, kid: string, privateDIDKeyHex: String, opts?: {traceability?: boolean, alias?: string, type?: TKeyType}) {
    ...
    if (method === DIDMethods.DID_WEB) {
        options = {
            kid,
            keys: [{key: {privateKeyHex: privateDIDKeyHex, kid, type:opts?.type ?? 'Secp256r1'}, type: opts?.type ?? 'Secp256r1', isController: true} as IKeyOpts]
        }
