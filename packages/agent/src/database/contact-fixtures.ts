import {CorrelationIdentifierEnum, IdentityRoleEnum, NonPersistedIdentity, PartyTypeEnum} from '@sphereon/ssi-sdk.data-store'
...
export async function addContacts() {
  try {
    ...
    let identifier = await agent.didManagerCreate(existingDidConfig(false, 'w-auth', PRIVATE_DID1_KEY_HEX))
    ...
    identifier = await agent.didManagerCreate(existingDidConfig(false, 'h-auth', PRIVATE_DID2_KEY_HEX))
    ...
    identifier = await agent.didManagerCreate(existingDidConfig(false, 'k-auth', PRIVATE_DID3_KEY_HEX))
    ...
    identifier = await agent.didManagerCreate(existingDidConfig(false, 'stonebase-auth', PRIVATE_DID5_KEY_HEX, {traceability: true}))
    ...
    identifier = await agent.didManagerCreate(existingDidConfig(false, 'sgs-auth', PRIVATE_DID6_KEY_HEX, {traceability: true}))
...
function existingDidConfig(anchor: boolean = false, kid: string, privateDIDKeyHex: String, opts?: {traceability?: boolean}) {
