...
export async function getDefaultKid({
  ...
  let keys = await mapIdentifierKeysToDocWithJwkSupport(
    identifier,
    verificationMethodName ?? "assertionMethod",
    context,
    didDocument,
  );
  if (
    keys.length === 0 &&
    (verificationMethodFallback === undefined || verificationMethodFallback)
  ) {
...
export async function getOrCreateDIDWebFromEnv(): Promise<IDIDResult[]> {
  ...
  if (identifier) {
    ...
    if (
      ...
      throw Error(
        "Not all required environment variables are present for importing a did:web from environment",
      );
    }
...
export async function getOrCreateDIDsFromFS(): Promise<IDIDResult[]> {
  ...
  const result = didOptConfigs.asArray.map(async (opts: IDIDResult) => {
    ...
    if (identifier) {
      ...
      console.log(
        `No identifier for DID ${did} exists yet. Will create the DID...`,
      );
      ...
      if (!did) {
        ...
        console.error(
          'Please adjust your did config file and add the "did" value to it: "did": "' +
            identifier.did +
            '"',
        );
