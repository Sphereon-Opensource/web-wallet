...
export async function getDefaultDID(): Promise<string | undefined> {
    ...
    return agent.didManagerFind().then((ids) => {
        if (!ids || ids.length === 0) {
            return Promise.reject(Error('Could not find a suitable default did identifier. (did:web:localhost is not suitable because RSA keys are not supported)'))
        }
