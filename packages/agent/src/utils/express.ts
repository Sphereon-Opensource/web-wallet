...
export function expressBuilder(opts?: {
  ...
  const cookieSigningKey =
    opts?.server?.cookieSigningKey ?? process.env.COOKIE_SIGNING_KEY;
  const hostname = opts?.server?.hostname ?? INTERNAL_HOSTNAME_OR_IP;
  const basePath = opts?.server?.basePath ?? process.env.BASE_PATH ?? "";
  const usePassport =
    opts?.auth?.authentication?.usePassport ?? AUTHENTICATION_ENABLED;
  ...
  const builder = ExpressBuilder.fromServerOpts({ ...serverOpts, envVarPrefix })
    ...
    .withPassportAuth(
      usePassport,
      opts?.auth?.authentication?.initializeOptions,
    );
