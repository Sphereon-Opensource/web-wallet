...
export async function getOrCreateConfiguredStatusList(args?: {
  issuer?: string;
  keyRef?: string;
}) {
  ...
  if (statusList) {
    console.log(
      `Existing status list found id ${STATUS_LIST_ID}, purpose ${statusList.statusPurpose} and length ${statusList.length}`,
    );
  } else {
    ...
    console.log(
      `New status list created with id ${STATUS_LIST_ID}, purpose ${STATUS_LIST_PURPOSE} and length ${STATUS_LIST_LENGTH}`,
    );
  }
}
