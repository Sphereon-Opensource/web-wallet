...
import {Siopv2MachineEvents, Siopv2MachineInterpreter, Siopv2MachineState, Siopv2MachineStates} from '@sphereon/ssi-sdk.siopv2-oid4vp-op-auth'
...
const navigateError = async (args: Siopv2NavigationArgs): Promise<void> => {
  ...
  console.log('navigateError', error)
...
const retrieveContact = async (siopv2Machine: Siopv2MachineInterpreter, machineState: Siopv2MachineState) => {
  ...
  await agent
    ...
    .then(party => {
      machineState.context.contact = party
    })
    .catch(error => Promise.reject(Error(`Unable to create contact. Error: ${error}`)))
}
...
// TODO maybe useful option to have:
//  vpStateCallbacks.set([Siopv2MachineStates.createConfig, Siopv2MachineStates.getSiopRequest,Siopv2MachineStates.sendResponse] , showLoading)
