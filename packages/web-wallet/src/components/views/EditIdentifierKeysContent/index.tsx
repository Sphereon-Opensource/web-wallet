...
const EditIdentifierKeysContent: FC = (): ReactElement => {
  ...
  const onAddKey = async (): Promise<void> => {
    ...
    if (action === 'select') {
      ...
      newKey = {
        ...
        readonly: false,
        capability: identifierCapability.add.keyTypes?.find(keyCap => keyCap.keyType === keyInfo.type),
      }
    } else {
      ...
      newKey = {
        ...
        readonly: false,
        capability: identifierCapability.add.keyTypes?.find(keyCap => keyCap.keyType === type),
      }
    }
  ...
  const getKeyElements = (): Array<ReactElement> => {
    return keys.map(key => {
      ...
      // In edit mode, allow deletion of all keys (don't check readonly flag)
      return <SelectionField key={key.id} value={key.alias} details={details} onRemove={onRemove} />
    })
  }
      ...
      <div className={style.contentContainer}>
        {keys.length === 0 && (
          <div className={style.formContainer}>
            <div>
              <div className={style.titleCaption}>{translate('edit_identifier_keys_title')}</div>
              <div className={style.descriptionCaption}>{translate('edit_identifier_keys_description')}</div>
            </div>
            <FormView
              data={keyData?.data}
              schema={schema}
            ...
            />
            <PrimaryButton
              caption={translate('edit_identifier_keys_add_key_label')}
              onClick={onAddKey}
              icon={ButtonIcon.ADD}
        ...
        {keys.length > 0 && (
          <div className={style.contentContainer}>
            ...
            {!maxKeysReached && (
              <div className={style.formContainer}>
                <div className={style.addTitleCaption}>{translate('edit_identifier_keys_add_key_title')}</div>
                <FormView
                  data={keyData?.data}
                  schema={schema}
                  uiSchema={addKeyUISchema}
...
export default EditIdentifierKeysContent
