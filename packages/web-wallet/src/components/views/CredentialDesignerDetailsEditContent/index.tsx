...
const CredentialDesignerDetailsEditContent: FC = (): ReactElement => {
  ...
  const {isAdvancedMode, onModeChange, credentialDesignerDetailsFormData, onCredentialDesignerDetailsFormDataChange, editData} =
  ...
  const advancedValidator = (content: string): string | null => {
    try {
      ...
      const allowedFormats = ['jwt_vc', 'ldp_vc', 'vc+jwt', 'dc+sd-jwt', 'jwt', 'ldp', 'mso_mdoc'] as const
