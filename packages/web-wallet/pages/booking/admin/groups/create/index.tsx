...
const GroupCreatePage: FC = (): ReactElement => {
  ...
  const {mutate: createGroup, isLoading: isCreating} = useCreate()
  ...
  // Filter resources that don't belong to a group yet (or belong to this category)
  ...
  const toggleResource = (resourceId: string) => {
    if (formData.resourceIds.includes(resourceId)) {
      setFormData({
        ...formData,
        resourceIds: formData.resourceIds.filter(id => id !== resourceId),
      })
    } else {
      setFormData({
        ...formData,
  ...
  const getButtonCaption = (): string => {
    if (step === maxSteps) {
      return isCreating ? 'Creating...' : 'Create Group'
    }
  ...
  return (
    <div className={style.container}>
      <PageHeaderBar path="Booking / Admin / Groups / Create" />
      <div className={style.contentContainer}>
          ...
          <div className={style.formCard}>
            ...
            {step === 1 && (
              <>
                ...
                <p className={style.stepDescription}>Enter the name and details for this resource group.</p>
                ...
                <div className={style.formGroup}>
                  ...
                  <select
                    ...
                    onChange={e => setFormData({
                      ...
                      // Reset resource selection when category changes
                      resourceIds: [],
                    })}
                  >
              ...
              <>
                ...
                <div className={style.inheritanceOption}>
                  <label className={style.radioLabel}>
                    ...
                    />
                    <div className={style.radioContent}>
                      <span className={style.radioTitle}>Assign a specific schedule set</span>
                      <span className={style.radioDescription}>
                ...
                {!formData.useInheritedSchedule && (
                  <div className={style.scheduleSetsList}>
                    {scheduleSets.length === 0 ? (
                      <p className={style.noSetsMessage}>No schedule sets available. Create one first.</p>
                    ) : (
            ...
            {step === 5 && (
              <>
                <h2>Review & Create</h2>
                <p className={style.stepDescription}>Review the group details before creating.</p>
        ...
        <ProgressStepIndicator
          steps={[
            ...
            {
              title: 'Schedule',
              description: 'Assign schedule set',
            },
            {
              title: 'Policy',
